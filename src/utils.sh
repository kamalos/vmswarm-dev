...
check_prerequisites() {
  ...
  if [[ $missing_deps -eq 1 ]]; then
    read -p "Required dependencies (qemu-kvm, libvirt-daemon-system, virtinst) are missing. Do you want to install them now? (y/n): " install_deps
    if [[ "$install_deps" == "y" || "$install_deps" == "Y" ]]; then
      sudo apt update && sudo apt install -y qemu-kvm libvirt-daemon-system virtinst
    else
      log_err $ERR_TOOL_MISSING "Required tools (virsh/virt-install) are missing."
    fi
  fi
